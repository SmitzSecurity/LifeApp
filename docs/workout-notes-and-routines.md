...
Apply **only migration 0009_routine_builder.sql** before deploying this feature. Migrations0000–0008 are already complete. The new table stores routine requests/results and extends life_ai_usage to include them alongside analyses and archived usage. Existing report, resource, consent and auth rows are untouched. The existing daily eligibility view and hand-authored reminder trigger are preserved.
