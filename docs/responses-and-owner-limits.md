...
The Responses workspace fills the available height between navigation bars: roughly 80% for today's editor and 20% for saved responses. Both areas scroll independently, with Save/Cancel always available. Short viewports keep usable minimum heights. History retains 60px rows and 44px edit/delete targets. Opening Search temporarily gives history more room; closing Search returns to the normal split. Pagination and confirmation dialogs remain available.

The small round microphone inside the journal box starts browser dictation only when clicked. Words appear as the browser processes them, including provisional text; later results replace that phrase without duplication. Existing typed text remains before the new speech. Tap the microphone again to stop, then review and Save. The journal and date cannot be edited during recording. Cancel, navigation and unmount abort recording and ignore queued callbacks. Permission denial and the 6,000-character limit preserve existing text. Unsupported browsers show a disabled microphone and suggest keyboard dictation. Availability and permissions depend on the browser; this does not call Gemini or consume LifeApp AI credits.

Repeatable browser verification: `node scripts/browser-smoke.mjs --calm --editing --analysis --history --dictation` serves a synthetic speech implementation and visible controls from the loopback-only fixture. No real microphone, production data or provider access is involved. Verify provisional text, corrections and final-result replacement, Save, Cancel/late results, switching entries, denial, length limits and unsupported fallback. Close the synthetic control panel before clicking underlying form buttons; keyboard activation also avoids its overlay.
