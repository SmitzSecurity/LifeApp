...
- Full-report emails use a separate opt-in, verified recipients, an atomic outbox and unsubscribe controls. Activation requires migration 0007 and the sender/runtime configuration in `docs/report-emails.md`. Email delivery never regenerates reports or grants automatic-analysis consent. The calmer check-in layout places the journal first and folds optional detail into expandable sections.
