...
import {themeVariables,palettes} from '../lib/life/appearance.ts';
...
test('standalone Worker uses Google sign-in and rejects forged Sites identity',async()=>{
 ...
 const values={},style={setProperty:(k,v)=>values[k]=v};runInNewContext(script,{localStorage:{getItem:()=>JSON.stringify({mode:'light',custom:{light:{primary:'#224466'}}})},document:{documentElement:{style,dataset:{}}}});assert.deepEqual(values,themeVariables({...palettes.light,primary:'#224466'}));
