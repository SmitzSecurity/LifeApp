...
test('standalone Worker uses Google sign-in and rejects forged Sites identity',async()=>{
 ...
 const signIn=await mf.dispatchFetch('https://life.test/sign-in',{headers});assert.equal(signIn.status,200);const html=await signIn.text();assert.match(html,/Continue with Google/);
 const script=[...html.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/g)].map(m=>m[1]).find(s=>s.includes('lifeapp-appearance-v1'));assert.ok(script,'Compiled page includes the pre-paint theme bootstrap');
 for(const typography of [undefined,{font:'serif',scale:130},{font:'url(https://invalid.test)',scale:999}]){
  const values={},style={setProperty:(k,v)=>values[k]=v};runInNewContext(script,{localStorage:{getItem:()=>JSON.stringify({mode:'light',custom:{light:{primary:'#224466'}},typography})},document:{documentElement:{style,dataset:{}}}});assert.deepEqual(values,{...themeVariables({...palettes.light,primary:'#224466'}),...typographyVariables(typography?.font==='serif'?typography:undefined)});
 }
