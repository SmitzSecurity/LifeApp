...
test('Gemini budget requests use a fixed extraction instruction, JSON schema and inline image rather than arbitrary URLs',async()=>{
 ...
 await provider.generate('{}','budget',png);assert.match(body.systemInstruction.parts[0].text,/You organize a pasted budget/);assert.deepEqual(body.contents[0].parts[1],{inlineData:png});assert.equal(body.generationConfig.responseMimeType,'application/json');assert.deepEqual(body.generationConfig.responseJsonSchema,budgetOutputSchema);assert.equal(body.generationConfig.responseFormat,undefined);assert.equal(body.generationConfig.responseSchema,undefined);
});
