...
export async function readActivity(db:Database,userId:string,from:string,through:string):Promise<ActivityRecords>{
 ...
 return {transactions:records.filter(r=>r.kind==='transaction'),workouts:records.filter(r=>r.kind==='workout'),cardio:records.filter(r=>r.kind==='cardio'),workoutNotes:records.filter(r=>r.kind==='workout-note')};
}
export function activityTotals(records:ActivityRecords,from:string,through:string){
 ...
 return {from,through,workoutNotes:(records.workoutNotes||[]).filter(n=>inWindow(n.data.date)&&!n.data.voided).length,transactions:tx.length,spendingCents:total('expense'),incomeCents:total('income'),savingCents:total('saving')+total('investing'),strengthSessions:workouts.length,sets:workouts.reduce((sum,w)=>sum+w.data.sets.length,0),cardioSessions:cardio.length,cardioMinutes:cardio.reduce((sum,c)=>sum+c.data.minutes,0),cardioKm:Math.round(cardio.reduce((sum,c)=>sum+(c.data.distance??0)*(c.data.unit==='mi'?1.609344:c.data.unit==='m'?.001:1),0)*100)/100};
}
