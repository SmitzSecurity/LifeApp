...
export async function readActivity(db:Database,userId:string,from:string,through:string):Promise<ActivityRecords>{
 ...
 return {transactions:records.filter(r=>r.kind==='transaction'&&!r.data.planned),workouts:[...records.filter(r=>r.kind==='workout'),...structuredNotes(records.filter(r=>r.kind==='workout-note'))],cardio:records.filter(r=>r.kind==='cardio'),workoutNotes:records.filter(r=>r.kind==='workout-note')};
}
export function activityTotals(records:ActivityRecords,from:string,through:string){
 ...
 const tx=records.transactions.filter(t=>inWindow(t.data.date)&&!t.data.planned&&!t.data.voided&&!t.data.deleted),workouts=records.workouts.filter(w=>inWindow(w.data.date)&&!!w.data.finishedAt&&!w.data.deleted),cardio=records.cardio.filter(c=>inWindow(c.data.date)&&!c.data.voided&&!c.data.deleted);
