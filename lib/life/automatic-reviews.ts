...
export async function scheduledDailyReviews(env: AIEnvironment & { DB?: Database }, scheduledTime: number) {
  await scheduledReviewPlanning(env, scheduledTime);
  const settings = settingsForAI(env);
  if (!automaticAvailable(settings, new Date())) return;
  if (!env.DB) throw new Error('Automatic review database unavailable');
  // Bill/pricing checks use execution time, not a possibly delayed Cron timestamp.
