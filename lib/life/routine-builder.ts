...
export async function listRoutineBuilds(db:Database,userId:string,settings:AISettings,now:Date,purpose:'routine'|'workout'|'training'='routine'){
 ...
 return json({available:settings.enabled&&!!settings.provider&&now.valueOf()<Date.parse(PRICE_EXPIRES),builds:rows.results.map(publicBuild),...(purpose==='workout'?{recovery:await availableWorkoutRecovery(db,userId,now)}:{})});
}
