...
const publicBuild=(row:BuildRow)=>({description:JSON.parse(row.input_snapshot).description,...(row.request_id.startsWith('budget:')?{month:JSON.parse(row.input_snapshot).month,...(JSON.parse(row.input_snapshot).intent?{intent:JSON.parse(row.input_snapshot).intent}:{}),resolvedBlocker:!!row.resolvedBlocker}:{}),deleted:!!row.deleted,id:row.request_id.slice(row.request_id.indexOf(':')+1),status:row.status,errorCode:row.error_code,createdAt:row.created_at,result:row.result_json?(row.request_id.startsWith('budget:')?budgetBuildResult:row.request_id.startsWith('workout:')?workoutBuildResult:row.request_id.startsWith('training:')?trainingResult:routineBuildResult).parse(JSON.parse(row.result_json)):null});
const read=(db:Database,userId:string,id:string)=>db.prepare(`SELECT request_id,status,input_snapshot,result_json,created_at,error_code, (b.request_id LIKE 'budget:%' AND (${settledBudgetRecoverySQL('?1','substr(b.request_id,8)')} OR ${acknowledgedBudgetBlockerSQL('?1','substr(b.request_id,8)')})) AS resolvedBlocker FROM life_routine_builds b WHERE user_id=?1 AND request_id=?2`).bind(userId,id).first<BuildRow>();
export async function listRoutineBuilds(db:Database,userId:string,settings:AISettings,now:Date,purpose:'routine'|'workout'|'training'|'budget'='routine',summary=false){
 const rows=await db.prepare(`SELECT request_id,status,input_snapshot,result_json,created_at,error_code, NOT (${visibleBuildSQL('b')}) AS deleted, (b.request_id LIKE 'budget:%' AND (${settledBudgetRecoverySQL('?1','substr(b.request_id,8)')} OR ${acknowledgedBudgetBlockerSQL('?1','substr(b.request_id,8)')})) AS resolvedBlocker FROM life_routine_builds b WHERE user_id=?1 AND request_id LIKE ?2 ORDER BY created_at DESC LIMIT 30`).bind(userId,purpose+':%').all<BuildRow>();
