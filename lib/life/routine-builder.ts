...
export async function buildRoutines(db:Database,userId:string,body:unknown,settings:AISettings,now:Date,purpose:'routine'|'workout'|'training'|'budget'='routine',evidence?:unknown){
 ...
 try{
  ...
  const diagnose=purpose==='budget'&&owner?.userId===userId&&now.valueOf()<Date.parse(owner.expiresAt)&&now.valueOf()<Date.parse('2026-09-17T00:00:00Z');
  ...
  if(row&&!draft&&diagnose&&!JSON.parse(row.input_snapshot).purged){
   const visible=await db.prepare(`SELECT request_id FROM life_routine_builds b WHERE user_id=?1 AND request_id=?2 AND (${visibleBuildSQL('b')})`).bind(userId,id).first();
   if(visible)return json({error:'Owner format diagnostic: '+result.text.slice(0,6000)},422);
  }
