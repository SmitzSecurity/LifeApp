...
export async function buildRoutines(db:Database,userId:string,body:unknown,settings:AISettings,now:Date,purpose:'routine'|'workout'|'training'|'budget'='routine',evidence?:unknown){
 ...
 const matches=(row:BuildRow)=>{const saved=JSON.parse(row.input_snapshot);return saved.description===input.text&&saved.recoveryOf===input.recoveryOf&&(!budget||saved.month===budget.month&&JSON.stringify(saved.image)===JSON.stringify(image));};
 ...
 const snapshot=JSON.stringify(budget?{description:input.text,month:budget.month,...(image?{image}:{}),moneyGoals:{goal:JSON.parse(profile.payload).moduleGoals?.money||'',budget:JSON.parse(profile.payload).budgetGoals||null}}:{description:input.text,...(input.recoveryOf?{recoveryOf:input.recoveryOf}:{}),...(purpose==='training'?{training:evidence}:{}),movementGoal:JSON.parse(profile.payload).moduleGoals?.fitness||'',defaultGoal:'Muscle growth with strength secondary',exerciseGuidance:purpose==='training'?[]:exercisePresets});
 if(new TextEncoder().encode(snapshot+(purpose==='budget'?budgetInstruction:purpose==='routine'?routineInstruction:purpose==='workout'?workoutInstruction:trainingInstruction)).length>MAX_INPUT_BYTES)return json({error:'Shorten the description before building.'},413);
