...
export function buildReviewContext(input:{profile:Profile;cadence?:Cadence;priorReviews?:ReviewRecord[];from:string;through:string;entries:Entry[];budget?:Saved<Budget>;transactions?:Saved<Transaction>[];workouts?:Saved<Workout>[];cardio?:Saved<Cardio>[];workoutNotes?:Saved<WorkoutNote>[]}){
 ...
 money:profile.modules.includes('money')&&input.budget?{month:input.budget.id,scope:'Full calendar month; distinguish this from the journal review window.',goals:profile.budgetGoals||input.budget.data.goals,summary:budgetSummary(input.budget.data,(input.transactions||[]).filter(t=>!t.data.planned),input.budget.id)}:null,
