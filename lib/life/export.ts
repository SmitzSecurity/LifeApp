...
export async function exportAccount(db:Database,userId:string,now:Date){
 ...
 const tables=[['entries','life_entries'],['resources','life_resources'],['reviews','life_ai_reviews'],['routineBuilds','life_routine_builds'],['trash','life_trash']] as const;
 const data:Record<string,unknown>={profile};
 let estimatedBytes=65536;
 for(const [name,table] of tables){
  const content=table==='life_trash'?"kind||record_id||deleted_at||COALESCE(purged_at,'')":table==='life_ai_reviews'?"COALESCE(input_snapshot,'')||COALESCE(report_text,'')":table==='life_routine_builds'?"COALESCE(input_snapshot,'')||COALESCE(result_json,'')":"payload";
