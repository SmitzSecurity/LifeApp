...
export async function readDebtPayments(request:Request,db:Database,userId:string,now=new Date()){
 ...
 const suppressedOccurrences=await readSuppressedOccurrences(db,userId,month.data,now);
 if(!ids.length)return reply({records:[],suppressedOccurrences});
 ...
 return reply({records:result.results.map(r=>({id:r.resource_id,data:transactionSchema.parse(JSON.parse(r.payload)),version:r.version,updatedAt:r.updated_at})),suppressedOccurrences});
}
