...
export const categorySchema=z.object({id:uuid,name:title,limitCents:cents}).strict();
const recurringSchema=z.object({id:uuid,title,kind:z.enum(['expense','income']),amountCents:cents.refine(n=>n>0),categoryId:z.string(),day:z.number().int().min(1).max(31),active:z.boolean().default(true)}).strict();
...
export const resourceKind=z.enum(['budget','transaction','routine','workout']);
export type ResourceKind=z.infer<typeof resourceKind>;
export const resourceSchemas={budget:budgetSchema,transaction:transactionSchema,routine:routineSchema,workout:workoutSchema};
...
export function budgetSummary(plan:Budget,transactions:Saved<Transaction>[],month:string){
 ...
 const due=plan.recurring.filter(r=>r.active).map(r=>({...r,date:dueDate(month,r.day),id:occurrenceId(month,r.id),recorded:live.some(t=>t.data.recurringId===r.id)}));
