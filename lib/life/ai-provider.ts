...
export type AIProvider={generate:(input:string,purpose?:'routine')=>Promise<AIResult>};
...
export function geminiProvider(key:string,fetcher:typeof fetch=fetch):AIProvider{return {async generate(input,purpose){
 const instruction=purpose==='routine'?routineInstruction:systemInstruction;
