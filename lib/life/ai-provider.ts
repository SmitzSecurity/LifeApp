import {budgetInstruction,budgetOutputSchema,type BudgetImage} from './budget-build-schema.ts';
...
export type AIProvider={generate:(input:string,purpose?:'routine'|'workout'|'training'|'budget',image?:BudgetImage)=>Promise<AIResult>};
...
export function geminiProvider(key:string,fetcher:typeof fetch=fetch):AIProvider{return {async generate(input,purpose,image){
 ...
 const response=await fetcher(`https://generativelanguage.googleapis.com/v1beta/models/${AI_MODEL}:generateContent`,{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':key},body:JSON.stringify({systemInstruction:{parts:[{text:instruction}]},contents:[{role:'user',parts:[{text:input},...(purpose==='budget'&&image?[{inlineData:image}]:[])]}],generationConfig:{candidateCount:1,maxOutputTokens:purpose==='routine'||purpose==='workout'||purpose==='budget'?8192:MAX_OUTPUT_TOKENS,thinkingConfig:{thinkingLevel:'low'},...(purpose==='budget'?{responseFormat:{text:{mimeType:'application/json',schema:budgetOutputSchema}}}:purpose==='workout'?{responseFormat:{text:{mimeType:'application/json',schema:workoutOutputSchema}}}:{})}}),signal:AbortSignal.timeout(55000)});
