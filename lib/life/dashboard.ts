...
export async function readDashboard(db:Database,userId:string,now:Date){
 ...
 const reports=await db.prepare(`SELECT cadence,MAX(entry_date) AS date FROM life_ai_reviews WHERE user_id=?1 AND cadence<>'daily' AND status='complete' AND ${visibleAnalysisSQL()} GROUP BY cadence`).bind(userId).all<{cadence:PeriodCadence;date:string}>();
