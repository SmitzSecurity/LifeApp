...
import { nextSet, restRemaining, workoutTotals, routineSchema, type Routine, type Workout, type Saved } from '@/lib/life/modules';
...
import {createWorkoutSession,extendWorkoutRest,skipCurrentSet,sameRoutinePlan} from '@/lib/life/workout-session';
...
export default function Workouts({profile,onDirty,active:visible=true,refreshKey=0}:{refreshKey?:number;active?:boolean;profile:Profile;onDirty:(v:boolean)=>void}){
 ...
 async function saveRoutine(){const record=routinePending||routine;if(!record||writeLock.current)return;writeLock.current=true;dataEpoch.current++;setBusy(true);setRoutineError('');setRoutinePending(record);try{const saved=await saveRecord('routine',record);setRoutines(prev=>[saved,...prev.filter(r=>r.id!==saved.id)]);setSelectedId(saved.id);setRoutine(null);setRoutinePending(null);setRoutineDirty(false);setPanel(null);setNotice('');}catch(e){setRoutineError((e as Error).message);if((e as {status?:number}).status&&((e as {status:number}).status<500))setRoutinePending(null);}finally{writeLock.current=false;setBusy(false);}}
 async function persistWorkout(record:Saved<Workout>,effect:'preserve'|'advance'|'end'='preserve'){
  ...
  try{
   ...
   if((e as {status?:number}).status&&((e as {status:number}).status<500))setPending(null);
  }finally{writeLock.current=false;setBusy(false);}
 }
 function start(r:Saved<Routine>){if(active||busy||pending||startPending||writeLock.current)return;try{const record=createWorkoutSession(r,todayIn(profile.timezone));setStartChoice(null);setPanel(null);void persistWorkout(record,'advance');}catch{setStartError('Complete the workout name and exercise targets before starting.');}}
 ...
 function requestStart(r:Saved<Routine>,changed:boolean){if(!changed){start(r);return;}setStartChoice(structuredClone(r));setTemplateName((r.data.name===selected?.data.name?r.data.name+' (copy)':r.data.name).slice(0,100));setStartError('');setPanel('start-options');}
 async function saveTemplateAndStart(mode:'new'|'overwrite'|'retry'){
  ...
  if(mode!=='retry'){
   if(!startChoice||startPending||busy)return;
   ...
   attempt={sourceId:startChoice.id,record:mode==='new'?{id:crypto.randomUUID(),version:0,data:{...parsed.data,exercises:parsed.data.exercises.map(exercise=>({...exercise,id:crypto.randomUUID()}))}}:{...startChoice,data:parsed.data}};
  }
  if(!attempt)return;
  ...
  try{saved=await saveRecord('routine',attempt.record);setRoutines(previous=>[saved!,...previous.filter(r=>r.id!==saved!.id)]);setSelectedId(saved.id);setStartPending(null);resetPreview(attempt.sourceId);setStartChoice(null);setPanel(null);}
  catch(e){setStartError((e as Error).message);if((e as {status?:number}).status&&((e as {status:number}).status<500))setStartPending(null);}
  finally{writeLock.current=false;setBusy(false);}
 ...
 {error&&<div role="alert" className="error">{error}{active&&!pending&&<Button variant="secondary" disabled={busy} onClick={()=>void reload(setDirty)}>{setDirty?'Discard set draft and reload':'Reload saved workout'}</Button>}</div>}{notice&&<p role="status" className="analysis-note">{notice}</p>}
 ...
 <WorkoutPanel open={panel==='start-options'} title="Start your edited workout" onClose={()=>{setStartChoice(null);setStartError('');setPanel(null);}} closeDisabled={busy||!!startPending||!!pending}>{startChoice&&<div className="workout-start-options"><p>Use these changes for today, or save them as a template for next time.</p>{startError&&<p className="error" role="alert">{startError}</p>}{startPending&&<p className="notice">{busy?'Saving template…':'Template save unconfirmed.'}{!busy&&<Button onClick={()=>void saveTemplateAndStart('retry')}>Retry template save & begin</Button>}</p>}<fieldset disabled={busy||!!startPending||!!pending}><Button onClick={()=>start(startChoice)}>Begin without saving template</Button><Button variant="secondary" onClick={()=>void saveTemplateAndStart('overwrite')}>Overwrite existing template & begin</Button><label className="compact-field">New template name<input maxLength={100} value={templateName} onChange={event=>setTemplateName(event.target.value)}/></label><Button variant="secondary" disabled={!templateName.trim()} onClick={()=>void saveTemplateAndStart('new')}>Save as new template & begin</Button><Button variant="ghost" onClick={()=>{setStartChoice(null);setPanel(null);}}>Back to preview</Button></fieldset></div>}</WorkoutPanel>
