...
export function useDraftSync(onSaved:(entry:Entry)=>void){
 ...
 const open=useCallback((entry:Entry)=>{
  if(writer.current?.dirty)throw new Error('Wait for this draft to sync before opening another day.');
 ...
 const edit=useCallback((entry:Entry)=>{writer.current?.edit({...entry,complete:false});if(timer.current)clearTimeout(timer.current);if(writer.current?.status!=='error')timer.current=setTimeout(()=>{void flush().catch(()=>{});},900);},[flush]);
 ...
 return {draft,status,syncError,dirty:status!=='saved',open,edit,flush,finish,discard};
}
