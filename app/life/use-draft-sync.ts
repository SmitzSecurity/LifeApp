...
export function useDraftSync(onSaved:(entry:Entry)=>void,autoSave=true){
 ...
 const edit=useCallback((entry:Entry)=>{writer.current?.edit({...entry,complete:false});if(timer.current)clearTimeout(timer.current);if(autoSave&&writer.current?.status!=='error'&&writer.current?.status!=='conflict')timer.current=setTimeout(()=>{void flush().catch(()=>{});},900);},[flush,autoSave]);
 const resolveConflict=useCallback((saved:Entry,keepChanges:boolean)=>{writer.current?.resolveConflict(saved,keepChanges);onSavedRef.current(saved);},[]);
 ...
 useEffect(()=>{
  if(!autoSave)return;
  const save=()=>{if(writer.current?.dirty&&writer.current.status!=='error'&&writer.current.status!=='conflict')void flush().catch(()=>{});};
 ...
 return {draft,status,syncError,dirty:status!=='saved',open,edit,flush,finish,commit,discard,resolveConflict};
}
