...
export default function WorkoutNotes({profile,onDirty,onSaved}:{profile:Profile;onDirty:(v:boolean)=>void;onSaved?:()=>void}){
 ...
 <div className="action-row written-save"><Button variant="ghost" disabled={operation.busy||!!operation.pending} onClick={cancel}>Cancel</Button><Button disabled={listening||operation.busy||!form.data.text.trim()} onClick={()=>void save()}>{operation.busy?'Saving…':operation.pending?'Retry save':form.data.structured?'Save workout':'Save workout note'}</Button></div>
