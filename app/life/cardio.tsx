...
export default function CardioLog({profile,onDirty,active=false,onSaved,onCancel}:{onCancel?:()=>void;onSaved?:()=>void;active?:boolean;profile:Profile;onDirty:(dirty:boolean)=>void}){
 ...
 async function save(record=form){if(!record)return;setBusy(true);setError('');setPending(record);try{const saved=await saveRecord('cardio',record);setRecords(prev=>[saved,...prev.filter(r=>r.id!==saved.id)]);if(form?.id===saved.id)setForm(null);setPending(null);setNotice(saved.data.deleted?'Cardio moved to Trash.':'Cardio saved.');onSaved?.();}catch(e){setError((e as Error).message);if((e as {status?:number}).status&&((e as {status:number}).status<500))setPending(null);}finally{setBusy(false);}}
