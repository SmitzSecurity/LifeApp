...
export default function BudgetPanel({profile,onDirty,onProfileSaved}:{profile:Profile;onDirty:(v:boolean)=>void;onProfileSaved:(p:Profile)=>void}){
 ...
 const dueHasDraft=(id:string)=>Object.entries(dirtyItems).some(([key,value])=>value&&key.startsWith('transaction-editor:')&&(key==='transaction-editor:due:'+id||key.startsWith('transaction-editor:category-due:')&&key.endsWith(':'+id)));
 ...
 return <section className="budget-workspace"><div className="budget-toolbar"><h2>Your month</h2><div className="compact-field"><MonthInput label="Month" aria-label="Budget month" required value={month} disabled={loading||dirty} title={dirty?'Save or cancel your open edits before changing months':undefined} onValueChange={month=>{if(month)setMonth(month);}}/></div></div>
 ...
 <BudgetSection id="allowances" title="Category allowances" className="module-card category-allowances" dirty={hasDirtyPrefix('category:')||plan.data.categories.some(c=>categoryHasDraft(c.id)||hasDirtyPrefix('transaction-action:'+c.id+':'))||hasDirtyPrefix('restore-category:')||hasDirtyPrefix('restore-recurring:')||!!dirtyItems['category-order']} actions={<Button variant="ghost" disabled={!plan.data.categories.length} onClick={()=>setOrdering(true)}aria-label="Edit categories">Edit</Button>}>
 {summary.categories.filter(c=>!c.archived||categoryHasDraft(c.id)).map(c=>{const category=plan.data.categories.find(x=>x.id===c.id)!;const recorded=transactions.filter(t=>t.data.kind==='expense'&&t.data.categoryId===c.id&&(!t.data.deleted&&!t.data.voided||hasTransactionDraft(c.id+':'+t.id))).sort((a,b)=>b.data.date.localeCompare(a.data.date));const upcoming=displayedDue.filter(r=>r.categoryId===c.id&&(!r.recorded||hasTransactionDraft('category-due:'+c.id+':'+r.id)));return <CategoryAllowance key={c.id} category={category} plan={plan} spent={c.spent} scheduled={c.scheduled} needsActual={summary.due.some(r=>r.categoryId===c.id&&r.variable&&!r.recorded&&r.date<=today)} onSave={saveItem} onDirty={reportDirty}>{upcoming.length>0&&<div className="category-upcoming"><h4>Upcoming & awaiting actuals</h4>{[...upcoming].sort((a,b)=>a.date.localeCompare(b.date)).map(r=><ExpectedItem key={r.id} {...txProps} scope={'category-due:'+c.id+':'+r.id} due={r} source={plan.data.recurring.find(x=>occurrenceId(month,x.id)===r.id)!} transaction={transactions.find(t=>t.id===r.id)} onEditRecurring={editRecurring}/>)}</div>}<h4>Recent transactions</h4>{recorded.length?recorded.map(t=><TransactionTile key={t.id} {...txProps} record={t} scope={c.id+':'+t.id}/>):<p className="muted">No recorded expenses in this category.</p>}</CategoryAllowance>;})}
 ...
 {!summary.due.length&&<p className="muted">No recurring payments are due this month.</p>}{[...displayedDue].sort((a,b)=>a.date.localeCompare(b.date)).map(r=><ExpectedItem key={r.id} {...txProps} due={r} source={plan.data.recurring.find(x=>occurrenceId(month,x.id)===r.id)!} transaction={transactions.find(t=>t.id===r.id)} onEditRecurring={editRecurring}/>)}</BudgetSection></div>
 <footer className="budget-footer"><span>Recorded totals exclude estimates.</span><div className="budget-footer-actions"><BudgetGoals profile={profile} fallback={plan.data.goals} onProfileSaved={onProfileSaved} onDirty={setGoalsDirty} disabled={false}/><span className="budget-goals-link"><button type="button" onClick={()=>setBuilding(true)}>Build with AI</button></span></div></footer>
...
function ExpectedItem({due,source,transaction,onEditRecurring,scope,...props}:{due:ReturnType<typeof budgetSummary>['due'][number];source:Recurring;transaction?:Saved<Transaction>;scope?:string;onEditRecurring:(item:Recurring)=>void;plan:Saved<Budget>;today:string;onSave:(t:Saved<Transaction>)=>Promise<Saved<Transaction>>;onDirty:DirtyReporter}){
