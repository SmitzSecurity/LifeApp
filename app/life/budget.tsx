...
import TransactionWindow from './transaction-window';
...
import {TransactionEditor,TransactionTile} from './budget-transactions';
...
export default function BudgetPanel({profile,onDirty,onProfileSaved}:{profile:Profile;onDirty:(v:boolean)=>void;onProfileSaved:(p:Profile)=>void}){
 ...
 function acceptImported(result:{records:Saved<Transaction>[];plans:Saved<Budget>[]}){
  for(const saved of result.plans)if(saved.id===month)acceptPlan(saved);
  const incoming=result.records.filter(record=>record.data.date.startsWith(month+'-'));
  setTransactions(previous=>{const merged=new Map(previous.map(record=>[record.id,record]));for(const record of incoming){const current=merged.get(record.id);if(!current||record.version>=current.version)merged.set(record.id,{...record,data:transactionSchema.parse(record.data)});}return [...merged.values()];});
...
<BudgetAnnualFund profile={profile} plan={plan} today={today} transactions={transactions} setup={recurring||building?null:fundSetup} onSetup={setFundSetup} onProfileSaved={onProfileSaved} onSave={saveTransaction} onDirty={reportDirty}/>
 <BudgetDebts dirtyItems={dirtyItems} plan={plan} today={today} transactions={transactions} onEdit={editRecurring} onAdd={()=>addRecurring(true)} onAI={()=>setBuilding('loans')} onSave={saveTransaction} onDirty={reportDirty}/>
<div className="budget-history"><BudgetSection id="history" title="Transaction history" className="module-card" dirty={hasDirtyPrefix('transaction-editor:history:')}>{!transactions.some(t=>!t.data.deleted&&(!t.data.planned||t.data.voided))&&<p className="muted">Your recorded transactions appear here.</p>}{transactions.filter(t=>!t.data.deleted&&(!t.data.planned||t.data.voided)||hasTransactionDraft('history:'+t.id)).sort((a,b)=>b.data.date.localeCompare(a.data.date)).map(t=><TransactionTile key={t.id} {...txProps} record={t} scope={'history:'+t.id}/>)}
 </BudgetSection></div>
 <footer className="budget-footer"><div className="budget-footer-actions"><BudgetGoals profile={profile} fallback={plan.data.goals} onProfileSaved={onProfileSaved} onDirty={setGoalsDirty} disabled={false}/><span className="budget-goals-link"><button type="button" onClick={()=>setBuilding('budget')}>Build with AI</button></span><span className="budget-goals-link"><button type="button" onClick={()=>setFundSetup('manage')}>Annual bills fund</button></span></div></footer>
 {building&&<BudgetBuilder intent={building==='loans'?'loans':undefined} plan={plan} onSave={saveItem} onClose={()=>setBuilding(null)} onDirty={reportDirty}/>}
 {logging&&<TransactionWindow plan={plan} today={today} annualFundEnabled={!!profile.annualFund?.enabled} onSaveTransaction={saveTransaction} onImported={acceptImported} onClose={()=>setLogging(false)} onDirty={reportDirty}/>}
 <Button className="budget-add-transaction" size="icon" aria-label="Log transactions" title="Log transactions" onClick={()=>setLogging(true)}><Plus aria-hidden="true"/></Button>
 {recurring&&(recurring.item.debt?<LoanDialog key={recurring.item.id} {...recurring} plan={plan} onSave={saveItem} onClose={()=>setRecurring(null)} onDirty={reportDirty}/>:<RecurringDialog key={recurring.item.id} {...recurring} plan={plan} annualFundEnabled={!!profile.annualFund?.enabled} onSave={saveItem} onClose={()=>setRecurring(null)} onDirty={reportDirty}/>)} {ordering&&<BudgetOrder plan={plan} onSave={saveItem} onClose={()=>setOrdering(false)} onDirty={reportDirty}/>}</div>}
 </section>;
}
