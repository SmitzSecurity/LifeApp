...
export default function BudgetPanel({profile,onDirty,onProfileSaved}:{profile:Profile;onDirty:(v:boolean)=>void;onProfileSaved:(p:Profile)=>void}){
 ...
 <div className="budget-columns"><section className="module-card quick-transaction"><h3>Quick transaction</h3>{quick&&<TransactionEditor key={quick.id} {...txProps} initial={quick} scope={'quick:'+quick.id} onCancel={resetQuick} onSaved={resetQuick}/>}</section>
 <section className="module-card category-allowances"><h3>Category allowances</h3>
 ...
 <section className="recurring-list" aria-label="Recurring income and expenses"><div className="section-heading"><h3>Recurring income & expenses</h3><Button variant="ghost" disabled={plan.data.recurring.length>=60} onClick={addRecurring}><Plus/>Add recurring item</Button></div>
 ...
 {plan.data.recurring.some(r=>r.deleted)&&<details className="deleted-items"><summary>Deleted monthly items</summary>{plan.data.recurring.filter(r=>r.deleted).map(r=><div className="ledger-row" key={r.id}><span>{r.title}</span><SaveItemButton id={'restore-recurring:'+r.id} label="Restore" item={{kind:'recurring',month,previous:r,item:{...r,deleted:false,active:true},...(!plan.version?{initial:plan.data}:{})} as BudgetItemChange} save={saveItem} onDirty={reportDirty}/></div>)}</details>}</section>
 </section></div>
 <div className="budget-columns budget-history"><section className="module-card"><h3>Transaction history</h3>{!transactions.some(t=>!t.data.deleted)&&<p className="muted">Your recorded transactions appear here.</p>}{transactions.filter(t=>!t.data.deleted||hasTransactionDraft('history:'+t.id)).sort((a,b)=>b.data.date.localeCompare(a.data.date)).map(t=><TransactionTile key={t.id} {...txProps} record={t} scope={'history:'+t.id}/>)}
 ...
 {recurring&&<RecurringDialog key={recurring.item.id} {...recurring} plan={plan} onSave={saveItem} onClose={()=>setRecurring(null)} onDirty={reportDirty}/>}</div>}
 </section>;
}
