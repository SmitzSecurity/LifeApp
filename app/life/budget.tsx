...
export default function BudgetPanel({profile,onDirty,onProfileSaved}:{profile:Profile;onDirty:(v:boolean)=>void;onProfileSaved:(p:Profile)=>void}){
 ...
 <footer className="budget-footer"><div className="budget-footer-actions"><BudgetGoals profile={profile} fallback={plan.data.goals} onProfileSaved={onProfileSaved} onDirty={setGoalsDirty} disabled={false}/><span className="budget-goals-link"><button type="button" onClick={()=>setBuilding(true)}>Build with AI</button></span></div></footer>
...
type ExpectedPayment={id:string;title:string;kind:Transaction['kind'];amountCents:number;categoryId:string;date:string;variable:boolean;recorded:boolean;actualCents:number};
...
function ExpectedItem({due,source,transaction,onEditRecurring,scope,...props}:{due:ExpectedPayment;source?:Recurring;transaction?:Saved<Transaction>;scope?:string;onEditRecurring:(item:Recurring)=>void;plan:Saved<Budget>;today:string;onSave:(t:Saved<Transaction>)=>Promise<Saved<Transaction>>;onDirty:DirtyReporter}){
 const [initial,setInitial]=useState<Saved<Transaction>|null>(null),[confirming,setConfirming]=useState(false),[locked,setLocked]=useState(false);
 const daysUntil=Math.round((Date.parse(due.date+'T00:00:00Z')-Date.parse(props.today+'T00:00:00Z'))/86400000);
