...
export default function Settings({config,onChange,focusFields,habitFields,dirty,busy,onSave,onCancel,onDelete}:{config:Profile;onChange:(p:Profile)=>void;focusFields:ReactNode;habitFields:ReactNode;dirty:boolean;busy:boolean;onSave:()=>void;onCancel:()=>void;onDelete?:()=>void}){
 ...
 return <section className="settings-panel organized-settings"><div className="settings-fields"><div className="settings-fields-inner"><p className="settings-intro">Make LifeApp work for you.</p>
 ...
 </div></div><footer className="settings-save"><Button variant="ghost" disabled={busy||!dirty} onClick={onCancel}>Cancel</Button><Button disabled={busy||!dirty} onClick={onSave}>{busy?'Saving…':'Save'}</Button></footer>
 </section>;
}
