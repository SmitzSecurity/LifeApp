...
import {request,useUnsaved,useWorkoutCancel} from './shared';
...
export default function RoutineBuilder({onDirty,onReview,reviewDisabled,embedded=false}:{embedded?:boolean;onDirty:(v:boolean)=>void;onReview:(routine:Saved<Routine>)=>void;reviewDisabled:boolean}){
 ...
 const cancel=useWorkoutCancel(()=>{setText('');setError('');},busy||!!pending);
 ...
 async function remove(build:Build){setBusy(true);setError('');try{await request('',{action:'record-deletion',change:{kind:'build',id:'routine:'+build.id,deleted:!build.deleted}});await refresh();}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
 ...
 <div className="action-row"><Button disabled={busy||listening||!loaded||!available||(!pending&&(text.trim().length<10||unconfirmed))} onClick={()=>void generate()}>{busy?'Working…':pending?'Check / retry this build':'Build with AI'}</Button><Button variant="ghost" disabled={busy||listening} onClick={()=>void refresh()}>Refresh status</Button>{embedded&&<Button variant="ghost" disabled={busy||!!pending} onClick={cancel}>Cancel</Button>}{text&&<Button variant="ghost" disabled={busy||listening||!!pending} onClick={()=>setText('')}>Clear</Button>}</div>
 {builds.filter(build=>!build.deleted).map(build=><article className="routine-build-result" key={build.id}><div className="section-heading"><small>{build.deleted?'Trash':'Saved AI draft'}</small><Button variant="ghost" disabled={busy} onClick={()=>void remove(build)}>{build.deleted?'Restore':'Delete'}</Button></div>{build.deleted?null:build.result?<><h4>Routine drafts</h4>{build.result.notes&&<p className="builder-notes">{build.result.notes}</p>}{build.result.routines.map(r=><div className="routine-draft" key={r.id}><div className="section-heading"><strong>{r.data.name}</strong><Button variant="secondary" disabled={reviewDisabled||busy} onClick={()=>onReview(r)}>Review routine</Button></div><ul>{r.data.exercises.map(e=><li key={e.id}><span>{e.name}</span><small>{e.sets} × {repTarget(e)} · {e.restSeconds}s rest{e.load?` · ${e.load} ${e.unit}`:''}</small></li>)}</ul></div>)}<small className="muted">Each draft is saved to your routine list only after you review it and choose Save routine.</small></>:<p role="status">{build.status==='generating'?'Your routine build is in progress. Refresh status to check it.':build.status==='uncertain'?'This build could not be confirmed. It will not be sent again automatically. New builds are paused while its outcome is unknown.':'The response could not be turned into a valid routine. Your existing routines are unchanged. You can simplify the description and try again.'}</p>}</article>)}
