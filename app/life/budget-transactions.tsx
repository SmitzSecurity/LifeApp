...
export function TransactionEditor({initial,scope,variable=false,confirmation=false,onLockChange,onCancel,onSaved,plan,today,onSave,onDirty}:Props&{initial:Saved<Transaction>;scope:string;variable?:boolean;confirmation?:boolean;onLockChange?:(locked:boolean)=>void;onCancel:()=>void;onSaved:()=>void}){
 const [base]=useState(initial),[data,setData]=useState(()=>confirmation?{...initial.data,date:plan.id===today.slice(0,7)?today:initial.data.date,voided:false,deleted:false}:initial.data),[initialAmount]=useState(!initial.version&&!initial.data.recurringId&&!initial.data.planned&&!confirmation?'':((initial.data.incomeDetails?.grossCents??initial.data.amountCents)/100).toFixed(2)),[amount,setAmount]=useState(initialAmount);
 ...
 <label className="compact-field">Type<Choice label="Transaction type" value={data.kind} options={data.recurringId||data.incomeSourceId?transactionKinds.filter(k=>k.value===data.kind):transactionKinds} onChange={kind=>edit({kind:kind as Transaction['kind'],categoryId:''})}/></label>
 ...
 </div>{data.incomeDetails&&<><IncomePreview grossCents={Math.round(Number(amount)*100)} plan={data.incomeDetails.plan}/><p className="field-hint">Income records take-home pay. Planned savings and investments only count after you confirm their transfers.</p></>}{data.incomeSourceId&&<p className="field-hint">Record this only after you transfer the money. LifeApp does not move it for you.</p>}{variable&&<p className="field-hint">Estimated {money(base.data.amountCents)}. Keep this amount or enter the actual amount.</p>}{planned&&<p className="field-hint">This is a planned item. It stays in expected payments until you confirm it.</p>}<label className="compact-field">Description<input maxLength={300} placeholder="e.g. Lunch" value={data.note} onChange={e=>edit({note:e.target.value})}/></label></fieldset>
