...
export default function BudgetGoals({profile,fallback,onProfileSaved,onDirty,disabled}:{profile:Profile;fallback:Budget['goals'];onProfileSaved:(p:Profile)=>void;onDirty:(v:boolean)=>void;disabled:boolean}){
 ...
 <Dialog open={open} onOpenChange={value=>{if(!busy){setOpen(value);if(!value){setGoals(initial);setError('');}}}}><DialogContent className="budget-goals-dialog" showCloseButton={!busy} onInteractOutside={e=>e.preventDefault()}>
