...
export default function Dictation({value,onChange,onListening,disabled=false,maxLength=5000,compact=false}:{value:string;onChange:(text:string)=>void;onListening:(v:boolean)=>void;disabled?:boolean;maxLength?:number;compact?:boolean}){
 ...
 function start(){setError('');const w=window as SpeechWindow,Ctor=w.SpeechRecognition||w.webkitSpeechRecognition;if(!Ctor)return;const r=new Ctor(),prefix=value?value+(/\s$/.test(value)?'':'\n'):'';recognition.current=r;r.lang=navigator.language||'en-US';r.continuous=true;r.interimResults=false;
  ...
  r.onresult=event=>{if(recognition.current!==r)return;const transcript=Array.from(event.results).filter(x=>x.isFinal).map(x=>x[0].transcript).join(' ');if(!transcript)return;const text=prefix+transcript;if(text.length>maxLength){setError('The text limit was reached. Shorten the text before continuing dictation.');finish();r.abort();return;}callback.current(text);};
 ...
 return <div className={`dictation-controls${compact?' compact-dictation':''}`}><Button type="button" variant="ghost" disabled={!supported||disabled&&!listening} title={supported?'Uses your browser’s speech service.':'Use your keyboard’s microphone to dictate in this browser.'} aria-pressed={listening} onClick={()=>listening?recognition.current?.stop():start()}>{listening?<Square/>:<Mic/>}{listening?'Stop dictation':'Dictate'}</Button>{(!compact||!supported)&&<small>{supported?(listening?'Listening…':'Uses your browser’s speech service.'):'Use your keyboard’s microphone to dictate in this browser.'}</small>}{error&&<small role="status">{error}</small>}</div>;
}
