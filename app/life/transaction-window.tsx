...
export default function TransactionWindow({plan,today,annualFundEnabled,onSaveTransaction,onImported,onClose,onDirty}:Props){
 ...
 const status=useAIStatus<{builds:Build[];available:boolean;blockedReason?:string}>({
  ...
  onData:data=>{
   ...
   if(build&&build.status!=='generating'){
    generation.current++;pendingRef.current=null;setPending(null);setBusy(false);
 ...
 function close(){if(closeLocked)return;mounted.current=false;generation.current++;reviewGeneration.current++;status.stop();onClose();}
 ...
 async function review(build:Build){
  ...
  const ticket=++reviewGeneration.current;setReading(true);setError('');setMode('ai');
  try{
 ...
 async function generate(){
  ...
  const ticket=++generation.current;writeLock.current=true;pendingRef.current=input;setPending(input);setBusy(true);setError('');
  try{
 ...
 async function changeDate(row:TransactionReviewRow,date:string){
  if(!draft||!isCalendarDate(date)||date<'1900-01-01'||date>today)return;const monthId=date.slice(0,7),ticket=++reviewGeneration.current;
