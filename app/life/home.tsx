...
function Bars({values,label,movement=false}:{values:number[];label:string;movement?:boolean}){const max=Math.max(...values,1);return <svg style={{fill:movement?'var(--chart-2)':'var(--chart-1)'}} className="trend-chart" viewBox="0 0 200 56" role="img" aria-label={label}>{values.map((value,i)=><rect key={i} x={i*51} y={54-Math.max(2,value/max*48)} width="38" height={Math.max(2,value/max*48)} rx="3"/>)}</svg>;}
export default function Home({profile,entries,onDate,onPeriod,onSection,onBusy,onProfileSaved}:{profile:Profile;entries:Entry[];onDate:(date:string)=>void;onPeriod:(cadence:PeriodCadence,date:string)=>void;onSection:(section:string)=>void;onBusy:(busy:boolean)=>void;onProfileSaved:(profile:Profile)=>void}){
 ...
 return <div className="home-dashboard">
  ...
  <section aria-label="Budget and movement trends"><div className="trend-heading"><h2>Your recent rhythm</h2><small>Last four weeks · logged activity</small></div><div className="trend-cards">
   <button onClick={()=>onSection('budget')} className="trend-card"><span><Wallet/>Budget<ArrowUpRight/></span><strong>{hasMoney?money(latest?.spendingCents||0):'Start with a transaction'}</strong><small>{hasMoney?'Spending logged in the last 7 days':'Your spending trend will appear here.'}</small>{hasMoney&&<Bars values={trends.map(t=>t.spendingCents)} label={trends.map(t=>`${formatDate(t.from)} to ${formatDate(t.through)}: ${money(t.spendingCents)} spending logged`).join('; ')}/>}</button>
   <button onClick={()=>onSection('workouts')} className="trend-card"><span><Dumbbell/>Movement<ArrowUpRight/></span><strong>{hasMovement?`${(latest?.strengthSessions||0)+(latest?.cardioSessions||0)+(latest?.workoutNotes||0)} movement logs`:'Log your first activity'}</strong><small>{hasMovement?`${latest?.cardioMinutes||0} cardio minutes · last 7 days`:'Strength training and cardio, together.'}</small>{hasMovement&&<Bars movement values={trends.map(t=>t.strengthSessions+t.cardioSessions+(t.workoutNotes||0))} label={trends.map(t=>`${formatDate(t.from)} to ${formatDate(t.through)}: ${t.strengthSessions} strength and ${t.cardioSessions} cardio sessions and ${t.workoutNotes||0} written workout logs`).join('; ')}/>}</button>
  </div></section>
