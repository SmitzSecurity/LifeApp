...
export default function PeriodicSettings({setupDirty}:{setupDirty:boolean}){
 ...
 async function load(){try{const saved=await request('?periodic=1');setData(saved);}catch{setError('Could not load your automatic analysis choice.');}}
 useEffect(()=>{let current=true;void request("?periodic=1").then(saved=>{if(current)setData(saved);}).catch(()=>{if(current)setError("Could not load your automatic analysis choice.");});return()=>{current=false;};},[]);
