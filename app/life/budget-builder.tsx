...
export default function BudgetBuilder({plan,onSave,onClose,onDirty}:{plan:Saved<Budget>;onSave:(change:BudgetItemChange)=>Promise<Saved<Budget>>;onClose:()=>void;onDirty:DirtyReporter}){
 ...
 <DialogHeader><DialogTitle>{draft?'Review your budget':'Build budget with AI'}</DialogTitle><DialogDescription>{draft?`Review the items for ${formatMonth(plan.id)}. Existing allowances keep their current amounts unless you edit them here.`:'Paste your budget, or attach a screenshot. You’ll review everything before it is added.'}</DialogDescription></DialogHeader>
