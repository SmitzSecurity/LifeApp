...
export default function TrainingAnalysis({onBusy,subtitle}:{onBusy:(v:boolean)=>void;subtitle?:ReactNode}){
 ...
 async function generate(){const retrying=!!pending,id=pending||crypto.randomUUID(),ticket=generation.current.epoch+1;generation.current={epoch:ticket,id};const current=()=>mounted.current&&generation.current.epoch===ticket;setPending(id);setGenerating(true);setDismissed(false);setError('');try{const data=await request('',{action:'training-analysis',build:{requestId:id,consent:true}});if(!current())return;setBuilds(old=>[data.build,...old.filter(b=>b.id!==data.build.id)]);if(data.build.status!=='generating')setPending(null);await status.check();}catch(e){if(!current())return;setError((e as Error).message);if(definiteClientRejection((e as {status?:number}).status,retrying)){setPending(null);generation.current.id=null;}await status.check();}finally{if(current())setGenerating(false);}}
 function cancelWait(){setDismissed(true);}
 async function remove(b:Build){const retrying=!!deletePending,target=deletePending||b;setDeletePending(target);setBusy(true);setError('');try{await request('',{action:'record-deletion',change:{kind:'build',id:'training:'+target.id,deleted:!target.deleted}});if(!mounted.current)return;setDeletePending(null);await status.check();}catch(e){if(!mounted.current)return;setError((e as Error).message);if(definiteClientRejection((e as {status?:number}).status,retrying))setDeletePending(null);await status.check();}finally{if(mounted.current)setBusy(false);}}
