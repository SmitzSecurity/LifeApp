...
export default function AIReview({entry,date,cadence='daily',profile,synced,heading='Analysis',onBusy,onProfileSaved,onSettings,onGenerated}:Props){
 ...
 return <section className="analysis-card" aria-label={heading}>
  ...
  {data&&<div className="analysis-tools">{latest&&!shown&&<Button variant="ghost" disabled={busy} onClick={()=>void remove(latest)}>{latest.deleted?'Restore latest':'Delete'}</Button>}
   ...
   {(reports.length>1||reports.some(r=>r.deleted))&&<Button variant="ghost" disabled={busy} onClick={()=>setHistoryOpen(v=>!v)}><History/>History & Trash</Button>}
  </div>}
  ...
  {historyOpen&&<div className="analysis-versions">{reports.filter(r=>r.id!==shown?.id).map(r=><article key={r.id}><h3>{r.deleted?'Trash · ':''}Version {r.revision}</h3>{!r.deleted&&<AnalysisText text={r.text||''}/>}<Button variant="ghost" disabled={busy} onClick={()=>void remove(r)}>{r.deleted?'Restore':'Delete'}</Button></article>)}</div>}
