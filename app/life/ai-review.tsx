...
export default function AIReview({entry,date,cadence='daily',profile,synced,heading='Analysis',onBusy,onProfileSaved,onSettings,onGenerated}:Props){
 ...
 const [data,setData]=useState<State|null>(null),[error,setError]=useState(''),[busy,setBusy]=useState(false),[feedbackOpen,setFeedbackOpen]=useState(false),[feedback,setFeedback]=useState(''),[feedbackId,setFeedbackId]=useState(''),[requestId,setRequestId]=useState(''),[notice,setNotice]=useState('');
 ...
 useEffect(()=>{setData(null);setError('');setNotice('');setFeedback('');setFeedbackOpen(false);setRequestId(crypto.randomUUID());void refresh();},[targetDate,cadence]);
 ...
 async function remove(report:AnalysisReport){setBusy(true);setError('');try{await request('',{action:'record-deletion',change:{kind:'analysis',id:report.id,deleted:!report.deleted}});await refresh();onGenerated?.();setNotice(report.deleted?'Analysis restored.':'Analysis moved to Trash.');}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
 ...
 return <section className="analysis-card" aria-label={heading}>
  ...
  {data&&<div className="analysis-tools">{latest&&!shown&&!latest.deleted&&<Button variant="ghost" disabled={busy} onClick={()=>void remove(latest)}>{latest.deleted?'Restore latest':'Delete'}</Button>}
   {shown&&<Button variant="ghost" disabled={busy||!synced} onClick={()=>void remove(shown)}>Delete</Button>}
