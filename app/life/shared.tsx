...
export async function request(path:string,body?:unknown){const r=await fetch('/api/life'+path,{method:body?'POST':'GET',headers:body?{'Content-Type':'application/json'}:undefined,body:body?JSON.stringify(body):undefined,cache:'no-store'});const value=await r.json();if(!r.ok)throw new Error(value.error||'Could not save. Please try again.');return value;}
