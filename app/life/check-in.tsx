...
export default function CheckIn({profile,entry,busy,analysis,sync,onEdit,onDate,onSave,onCancel,onListening,cancelVoice}:Props){
 ...
 return <section className="response-editor" aria-label="Response form"><div className="response-fields">{analysis}
  ...
  <div className="response-actions"><span className="sr-only" role="status">{saved}{!sync.dirty&&entry.version>0?` · ${entry.complete?'Complete':'Draft'}`:''}</span><div className="response-save-buttons"><Button variant="ghost" disabled={busy||sync.status==='saving'||sync.status==='error'} onClick={onCancel}>Cancel</Button><Button variant="ghost" className="response-save" disabled={busy||listening||sync.status==='saving'||(!sync.dirty&&!entry.version)} onClick={onSave}>{sync.status==='saving'?<LoaderCircle className="spin"/>:null}Save</Button></div></div>
 </section>;
}
